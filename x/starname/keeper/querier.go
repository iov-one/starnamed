...
func queryOwnerAccounts(ctx sdk.Context, keeper *Keeper, owner sdk.AccAddress, start, end uint64, count bool) (*types.QueryOwnerAccountsResponse, error) {
	cursor, err := keeper.AccountStore(ctx).Query().Where().Index(types.AccountAdminIndex).Equals(owner).WithRange().Start(start).End(end).Do()
	if err != nil {
	...
	page, err := getPageResponse(count, keeper.AccountStore(ctx).Query().Where().Index(types.AccountAdminIndex).Equals(owner))
	if err != nil {
		return nil, sdkerrors.Wrapf(err, "'%s' caused error", bech32FromBytes(owner))
	}
